...
//            out_frame = "camera_depth_optical_frame";
        ...
        pcl::PointCloud<PointType> planar_object;


        // Identify the planar object
        pcl::SACSegmentation<PointType> segmentation;
        segmentation.setModelType(pcl::SACMODEL_PLANE);
        segmentation.setMethodType(pcl::SAC_RANSAC);
        segmentation.setMaxIterations(1000);
        segmentation.setDistanceThreshold(0.015);
        segmentation.setInputCloud((*input_tf_pcl_non_zero).makeShared());
        segmentation.segment(*inliers, coefficients);

        // Create the filtering object
        pcl::ExtractIndices<PointType> extract;
        extract.setInputCloud((*input_tf_pcl_non_zero).makeShared());
        ...
        extract.filter(*cloud_filteredZ);
