...
//        scene_filename_ = argv[filenames[1]];
        ...
        if (pcl::console::find_switch (argc, argv, "-k"))
            {
                std::cout << "SHOW KEY POINT IS SET TO TRUE!!" << std::endl;
...
void cloud_cb(const sensor_msgs::PointCloud2ConstPtr& cloud){
        ...
        if(ref_cloud_dict.empty())
            return;
        //load clouds
        model = ref_cloud_dict[0];
        ROS_INFO("WE ARE IN BUSINESS");
        ...
        ROS_ERROR_STREAM("MODEL NORMALS SIZE : " << model_normals->points.size());

        norm_est.setInputCloud (scene);
        norm_est.compute (*scene_normals);

        ROS_ERROR_STREAM("SCENE NORMALS SIZE : " << scene_normals->points.size());
        //
...
main (int argc, char** argv)
    {
        ...
        ros::init(argc, argv, "correspondence_grouping_node");
        ros::NodeHandle nh;
        ...
        if(client.call(srv))
            {
                ...
                int nb_objects = obj_cloud_vector.size();
                for (int obj_id=0; obj_id < nb_objects; obj_id++){
                        ...
                        pcl::PointCloud<PointType>::Ptr input_cloud_pcl(new pcl::PointCloud<PointType>);
